...
?>
               ...
               <div class="midde_cont">
                  <div class="container-fluid">
                     ...
                     <form method="POST" action="rentupdate.php">
                        <?php
                        $r = mysqli_query($con, "select * from tbl_roomtype where roomtype_id=$rid");
                        $ty = mysqli_fetch_assoc($r);
                        ?>
                        <div class="form-group">
                           <label for="exampleInputPassword1">Room Type</label>
                           <input type="text" class="form-control" name="room" id="exampleInputPassword1" placeholder="enter room type" value="<?php echo $ty['roomtype'] ?>" readonly>
                        </div>
                        <input type="hidden" class="form-control" name="rid" id="exampleInputPassword1" value="<?php echo $rid ?>" placeholder="enter room type">
                        <div class="form-group">
                           <label for="exampleInputPassword1">Room Rent</label>
                           <input type="number" class="form-control" name="rent" id="exampleInputPassword1" value="<?php echo $ty['room_rent'] ?>" placeholder="enter room type">
                        </div>
   ...
   if (isset($_POST['submit'])) {
      $rid = $_POST['rid'];
      $rent = $_POST['rent'];
      if (mysqli_query($con, "update tbl_roomtype set room_rent=$rent where roomtype_id=$rid")) {
         if (headers_sent()) {
            die('<script type="text/javascript">window.location.href="catroom.php?e=1"</script>');
         } else {
            header('location:catroom.php');
            die();
         }
      }
   }
